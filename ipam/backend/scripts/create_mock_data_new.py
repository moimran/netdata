...
def create_mock_data():
    ...
    with Session(engine) as session:
        ...
        print("Creating Aggregates...")
        aggregates = [
            Aggregate(prefix="10.0.0.0/8", description="RFC 1918 Private IPv4 Space - Class A", 
                     date_added=date.today(), rir_id=rirs[3].id),
            Aggregate(prefix="172.16.0.0/12", description="RFC 1918 Private IPv4 Space - Class B", 
                     date_added=date.today(), rir_id=rirs[3].id),
            Aggregate(prefix="192.168.0.0/16", description="RFC 1918 Private IPv4 Space - Class C", 
                     date_added=date.today(), rir_id=rirs[3].id)
        ]
        ...
        sites = [
            ...
            Site(name="NYC-OFF1", slug="nyc-off1", description="New York HQ Office",
                facility="Corporate HQ", physical_address="350 5th Avenue, New York, NY",
                latitude=40.7486, longitude=-73.9864, status="active",
                contact_name="Sarah Johnson", contact_email="sarah.j@example.com",
                contact_phone="+1-212-555-0300", tenant=tenant, region_id=sub_regions[0].id),
            Site(name="SFO-OFF1", slug="sfo-off1", description="San Francisco Office",
                facility="SF Branch Office", physical_address="101 California St, San Francisco, CA",
                latitude=37.7749, longitude=-122.4194, status="active",
                contact_name="Michael Brown", contact_email="michael.b@example.com",
                contact_phone="+1-415-555-0400", tenant=tenant, region_id=sub_regions[1].id)
        ]
        ...
        locations = [
            # NYC DC1 Locations
            Location(name="NYC-DC1-Floor1", slug="nyc-dc1-floor1", description="NYC DC1 Floor 1",
                    status="active", site_id=sites[0].id),
            Location(name="NYC-DC1-Floor2", slug="nyc-dc1-floor2", description="NYC DC1 Floor 2",
                    status="active", site_id=sites[0].id),
            # SJC DC1 Locations
            Location(name="SJC-DC1-Floor1", slug="sjc-dc1-floor1", description="SJC DC1 Floor 1",
                    status="active", site_id=sites[1].id),
            # LON DC1 Locations
            Location(name="LON-DC1-Floor1", slug="lon-dc1-floor1", description="LON DC1 Floor 1",
                    status="active", site_id=sites[2].id)
        ]
        ...
        sub_locations = [
            ...
            Location(name="NYC-DC1-F1-ServerRoom", slug="nyc-dc1-f1-server", 
                    description="NYC DC1 Floor 1 Server Room", status="active", 
                    site_id=sites[0].id, parent_id=locations[0].id),
            Location(name="NYC-DC1-F1-NetworkRoom", slug="nyc-dc1-f1-network", 
                    description="NYC DC1 Floor 1 Network Room", status="active", 
                    site_id=sites[0].id, parent_id=locations[0].id),
            # NYC DC1 Floor 2 Rooms
            Location(name="NYC-DC1-F2-ServerRoom", slug="nyc-dc1-f2-server", 
                    description="NYC DC1 Floor 2 Server Room", status="active", 
                    site_id=sites[0].id, parent_id=locations[1].id),
            # SJC DC1 Floor 1 Rooms
            Location(name="SJC-DC1-F1-ServerRoom", slug="sjc-dc1-f1-server", 
                    description="SJC DC1 Floor 1 Server Room", status="active", 
                    site_id=sites[1].id, parent_id=locations[2].id),
            # LON DC1 Floor 1 Rooms
            Location(name="LON-DC1-F1-ServerRoom", slug="lon-dc1-f1-server", 
                    description="LON DC1 Floor 1 Server Room", status="active", 
                    site_id=sites[2].id, parent_id=locations[3].id)
        ]
        ...
        # Create Management Prefixes from Aggregates
        print("Creating Management Prefixes...")
        mgmt_prefixes = [
            # Global Management - from 172.16.0.0/12 aggregate
            Prefix(name="Global Management", slug="global-mgmt", description="Global Management Network",
                  prefix="172.16.0.0/16", status=PrefixStatusEnum.ACTIVE, vrf=vrfs[2], role=roles[2],
                  tenant=tenant, vlan=vlans[2]),
            # NYC Office Management - from 172.16.0.0/12 aggregate
            Prefix(name="NYC Office Management", slug="nyc-office-mgmt", 
                  description="NYC Office Management Network", prefix="172.17.0.0/16", 
                  status=PrefixStatusEnum.ACTIVE, vrf=vrfs[2], role=roles[2],
                  site=sites[3], tenant=tenant, vlan=vlans[2]),
            # SFO Office Management - from 172.16.0.0/12 aggregate
            Prefix(name="SFO Office Management", slug="sfo-office-mgmt", 
                  description="SFO Office Management Network", prefix="172.18.0.0/16", 
                  status=PrefixStatusEnum.ACTIVE, vrf=vrfs[2], role=roles[2],
                  site=sites[4], tenant=tenant, vlan=vlans[2])
        ]
        session.add_all(mgmt_prefixes)
        session.commit()
        print(f"Created {len(mgmt_prefixes)} Management Prefixes")

        # Create DMZ Prefixes from Aggregates
        print("Creating DMZ Prefixes...")
        dmz_prefixes = [
            # NYC DMZ - from 192.168.0.0/16 aggregate
            Prefix(name="NYC DMZ", slug="nyc-dmz", description="NYC Data Center DMZ",
                  prefix="192.168.1.0/24", status=PrefixStatusEnum.ACTIVE, vrf=vrfs[3], 
                  role=roles[3], site=sites[0], tenant=tenant),
            # SJC DMZ - from 192.168.0.0/16 aggregate
            Prefix(name="SJC DMZ", slug="sjc-dmz", description="SJC Data Center DMZ",
                  prefix="192.168.2.0/24", status=PrefixStatusEnum.ACTIVE, vrf=vrfs[3], 
                  role=roles[3], site=sites[1], tenant=tenant),
            # LON DMZ - from 192.168.0.0/16 aggregate
            Prefix(name="LON DMZ", slug="lon-dmz", description="London Data Center DMZ",
                  prefix="192.168.3.0/24", status=PrefixStatusEnum.ACTIVE, vrf=vrfs[3], 
                  role=roles[3], site=sites[2], tenant=tenant)
        ]
        session.add_all(dmz_prefixes)
        session.commit()
        print(f"Created {len(dmz_prefixes)} DMZ Prefixes")

        # Create Development Prefixes from Aggregates
        print("Creating Development Prefixes...")
        dev_prefixes = [
            # NYC Development - from 172.16.0.0/12 aggregate
            Prefix(name="NYC Development", slug="nyc-dev", description="NYC Development Network",
                  prefix="172.20.0.0/16", status=PrefixStatusEnum.ACTIVE, vrf=vrfs[1], 
                  role=roles[1], site=sites[0], tenant=tenant, vlan=vlans[1]),
            # SJC Development - from 172.16.0.0/12 aggregate
            Prefix(name="SJC Development", slug="sjc-dev", description="SJC Development Network",
                  prefix="172.21.0.0/16", status=PrefixStatusEnum.ACTIVE, vrf=vrfs[1], 
                  role=roles[1], site=sites[1], tenant=tenant, vlan=vlans[1])
        ]
        ...
        ip_ranges = [
            ...
            # Production Ranges - from 10.1.0.0/16 prefix
            IPRange(name="NYC-DC Prod DB Servers", slug="nyc-dc-prod-db", 
                   description="NYC Production Database Servers", start_address="10.1.2.0", 
                   end_address="10.1.2.255", status=IPRangeStatusEnum.ACTIVE, 
                   vrf=vrfs[0], tenant=tenant),
            ...
            IPRange(name="SJC-DC Prod App Servers", slug="sjc-dc-prod-app", 
                   ...
                   vrf=vrfs[0], tenant=tenant),
            # Management Ranges - from 172.16.0.0/16 prefix
            IPRange(name="NYC Management Devices", slug="nyc-mgmt-dev", 
                   description="NYC Management Network Devices", start_address="172.16.1.0", 
                   end_address="172.16.1.255", status=IPRangeStatusEnum.ACTIVE, 
                   vrf=vrfs[2], tenant=tenant),
            # Management Ranges - from 172.16.0.0/16 prefix
            IPRange(name="SJC Management Devices", slug="sjc-mgmt-dev", 
                   description="SJC Management Network Devices", start_address="172.16.2.0", 
                   end_address="172.16.2.255", status=IPRangeStatusEnum.ACTIVE, 
                   vrf=vrfs[2], tenant=tenant),
            # DMZ Ranges - from 192.168.1.0/24 prefix
            IPRange(name="NYC DMZ Web Servers", slug="nyc-dmz-web", 
                   description="NYC DMZ Web Servers", start_address="192.168.1.0", 
                   end_address="192.168.1.127", status=IPRangeStatusEnum.ACTIVE, 
                   vrf=vrfs[3], tenant=tenant),
            # DMZ Ranges - from 192.168.2.0/24 prefix
            IPRange(name="SJC DMZ Web Servers", slug="sjc-dmz-web", 
                   description="SJC DMZ Web Servers", start_address="192.168.2.0", 
                   end_address="192.168.2.127", status=IPRangeStatusEnum.ACTIVE, 
                   vrf=vrfs[3], tenant=tenant)
        ]
        ...
        ip_addresses = [
            ...
            IPAddress(address="10.1.1.11/32", name="NYC-APP-02", status=IPAddressStatusEnum.ACTIVE,
                     ...
                     vrf=vrfs[0], tenant=tenant),
            IPAddress(address="10.1.2.10/32", name="NYC-DB-01", status=IPAddressStatusEnum.ACTIVE,
                     role=IPAddressRoleEnum.LOOPBACK, dns_name="nyc-db-01.example.com",
                     vrf=vrfs[0], tenant=tenant),
            ...
            IPAddress(address="10.2.1.10/32", name="SJC-APP-01", status=IPAddressStatusEnum.ACTIVE,
                     ...
                     vrf=vrfs[0], tenant=tenant),
            # Management IP Addresses - from 172.16.0.0/16 prefix
            IPAddress(address="172.16.1.10/32", name="NYC-MGMT-01", status=IPAddressStatusEnum.ACTIVE,
                     role=IPAddressRoleEnum.LOOPBACK, dns_name="nyc-mgmt-01.example.com",
                     vrf=vrfs[2], tenant=tenant),
            IPAddress(address="172.16.2.10/32", name="SJC-MGMT-01", status=IPAddressStatusEnum.ACTIVE,
                     role=IPAddressRoleEnum.LOOPBACK, dns_name="sjc-mgmt-01.example.com",
                     vrf=vrfs[2], tenant=tenant),
            # DMZ IP Addresses - from 192.168.1.0/24 prefix
            IPAddress(address="192.168.1.10/32", name="NYC-WEB-01", status=IPAddressStatusEnum.ACTIVE,
                     role=IPAddressRoleEnum.LOOPBACK, dns_name="nyc-web-01.example.com",
                     vrf=vrfs[3], tenant=tenant),
            IPAddress(address="192.168.1.11/32", name="NYC-WEB-02", status=IPAddressStatusEnum.ACTIVE,
                     role=IPAddressRoleEnum.LOOPBACK, dns_name="nyc-web-02.example.com",
                     vrf=vrfs[3], tenant=tenant),
            IPAddress(address="192.168.2.10/32", name="SJC-WEB-01", status=IPAddressStatusEnum.ACTIVE,
                     role=IPAddressRoleEnum.LOOPBACK, dns_name="sjc-web-01.example.com",
                     vrf=vrfs[3], tenant=tenant)
        ]
